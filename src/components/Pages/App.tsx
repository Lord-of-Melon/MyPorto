            ...
            <div className='relative inline-block'>
              <button onClick={() => setOpen (!open)} className='bg-blue-500 text-white text-base px-2 py-0.5 rounded-md hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-opacity-75'>
                <div className=' flex justify-between items-center'>
                  <div className='flex justify-center w-32'>{defaultText}</div> {dropdownIcon}
                </div>
              </button>
              ...
              {open && (
                <div className="absolute mt-0.5 w-42 rounded bg-white shadow-lg">
                  <ul>
                    <li className="flex items-center px-2 hover:bg-gray-100 cursor-pointer">
                        <button onClick={() => {setOpen (!open); setDefaultText('Home')}}  className='flex justify-start w-full text-base py-1 hover:scale-105'> Home </button>
                    </li>
                    <li className="flex items-center px-2 hover:bg-gray-100 cursor-pointer">
                        <button onClick={() => {setOpen (!open); setDefaultText('Gallery')}} className='flex justify-start w-full text-base py-1 hover:scale-105'> Gallery </button>
                    </li>
                    <li className="flex items-center px-2 hover:bg-gray-100 cursor-pointer">
                        <button onClick={() => {setOpen (!open); setDefaultText('Settings')}} className='flex justify-start w-full text-base py-1 hover:scale-105'> Settings </button>
                    </li>
                  </ul>
                </div>
              )}
